...
from ..exceptions import MountingError
...
class ServerMounter:
    ...
    async def mount_server(self, server_name: str, config: ServerConfig) -> bool:
        ...
        logger.info(f"Mounting server: {server_name}")
        ...
        try:
            ...
            if server_info.is_local:
                ...
                logger.error(f"Unsupported server configuration: {server_name}")
                return False
            ...
            if success:
                ...
                logger.info(f"Successfully mounted server: {server_name}")
                return True
            server_info.status = "failed"
            return False

        except Exception as e:
            logger.error(f"Failed to mount server {server_name}: {e}")
    ...
    async def _start_local_server(self, server_info: MountedServerInfo) -> bool:
        ...
        try:
            ...
            if process.poll() is not None:
                logger.error(f"Local server process failed to start: {server_info.name}")
                return False
            ...
            logger.error(f"Failed to start local server {server_info.name}: {e}")
            return False
    ...
    async def _start_remote_server(self, server_info: MountedServerInfo) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to connect to remote server {server_info.name}: {e}")
            return False
    ...
    async def _test_server_connection(self, server_info: MountedServerInfo) -> None:
        ...
        try:
            ...
            logger.error(f"Server connection test failed {server_info.name}: {e}")
            raise
    ...
    async def _mount_to_main_server(self, server_info: MountedServerInfo) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to mount server to main server {server_info.name}: {e}")
            raise

    async def unmount_server(self, server_name: str) -> bool:
        """Unmount specified server"""
        if server_name not in self.mounted_servers:
            logger.warning(f"Server not mounted: {server_name}")
            return False

        server_info = self.mounted_servers[server_name]
        logger.info(f"Unmounting server: {server_name}")
        ...
        try:
            ...
            logger.info(f"Successfully unmounted server: {server_name}")
            return True

        except Exception as e:
            logger.error(f"Failed to unmount server {server_name}: {e}")
            return False
    ...
    async def unmount_all_servers(self) -> None:
        ...
        logger.info(f"Successfully unmounted {success_count}/{len(results)} servers")
    ...
    async def _perform_health_checks(self) -> None:
        ...
        for server_info in self.mounted_servers.values():
            ...
            try:
                ...
                logger.warning(f"Server health check failed {server_info.name}: {e}")
                server_info.status = "failed"
                server_info.error_message = str(e)

                # Auto restart
                if self.auto_restart and server_info.restart_attempts < self.max_restart_attempts:
                    logger.info(f"Attempting to restart server {server_info.name}")
